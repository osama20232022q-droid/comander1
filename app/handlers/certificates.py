...
def _start_of_day_utc(days_ago: int = 0):
    ...
    return datetime(now.year, now.month, now.day, tzinfo=timezone.utc) - timedelta(days=days_ago)
...
async def create_certificate_if_earned(update: Update, cert_type: str) -> None:
    with get_session() as db:
        ...
        if cert_type == "daily":
            ...
            sessions = db.scalars(select(PomodoroSession).where(PomodoroSession.user_id == user.id, PomodoroSession.status == "finished", PomodoroSession.ended_at >= start)).all()
            today_hours = _hours(sessions)
            prev_start = _start_of_day_utc(7)
            prev_sessions = db.scalars(select(PomodoroSession).where(PomodoroSession.user_id == user.id, PomodoroSession.status == "finished", PomodoroSession.ended_at >= prev_start, PomodoroSession.ended_at < start)).all()
            ...
            if not earned:
                await update.effective_message.reply_text(f"لم تستحق شهادة اليوم بعد. ساعات اليوم: {today_hours:.1f}h. المطلوب: 3h على الأقل أو يوم أقوى من متوسطك.", reply_markup=certificate_keyboard())
                return
            hours = today_hours
            ...
            if not earned:
                await update.effective_message.reply_text(f"لم تستحق الشهادة الأسبوعية بعد. آخر 7 أيام: {hours:.1f}h / أيام فعالة: {active_days} / جلسات: {len(sessions)}.", reply_markup=certificate_keyboard())
                return
        ...
        html = certificate_html(user.profile.full_name if user.profile else user.first_name or "طالب", cert_type, hours, len(sessions), subjects_count, score, reason)
        cert = Certificate(user_id=user.id, title="شهادة يوم مميز" if cert_type == "daily" else "شهادة أسبوعية", html_content=html, hours=hours, score=score)
        db.add(cert)
        db.commit()
        db.refresh(cert)
    tmp = Path(tempfile.gettempdir()) / f"certificate_{cert.id}.html"
    tmp.write_text(html, encoding="utf-8")
    await update.effective_message.reply_document(tmp.open("rb"), filename=f"certificate_{cert.id}.html", caption="✅ تم منح الشهادة لأن الشرط تحقق.", reply_markup=certificate_keyboard())
...
async def list_certificates(update: Update) -> None:
    ...
    for cert in certs:
        ...
        await update.effective_message.reply_document(tmp.open("rb"), filename=f"certificate_{cert.id}.html", caption=f"{cert.title} — {cert.hours:.1f}h — Score {cert.score}")
