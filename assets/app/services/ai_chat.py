...
def _post_gemini(model: str, system_text: str, contents: list[dict[str, Any]], generation_config: dict[str, Any]) -> AIResult:
    ...
    try:
        ...
        try:
            ...
            body = str(e)
        error_code = "model_not_found" if e.code == 404 else "http_error"
        return AIResult(ok=False, error_code=error_code, text=f"صار خطأ من خدمة AI: {e.code}\n{body}")
    except TimeoutError:
...
async def generate_ai_reply(
    *,
    ...
    for model_name in models_to_try:
        for attempt in range(2):  # retry once on timeout before moving to next model
            result = await asyncio.to_thread(_post_gemini, model_name, system_text, contents, generation_config)
            if result.error_code != "timeout":
                break
        if result.ok or result.error_code != "model_not_found":
            break
